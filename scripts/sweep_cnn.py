...
SCRIPT_PATH = "NeurIPS-2025/S2-mnist-CNN.py"     # relative to TRAIN_ROOT
TRAIN_ROOT  = Path("~/analog_stress_test/analog-training").expanduser()
LOG_ROOT    = Path("~/analog_stress_test/logs/cnn_sweep").expanduser()

ALL_ALGOS = ["Analog SGD", "TT-v1", "TT-v2", "RL-v2"]

# Paper CNN MNIST hyperparams: tau=0.7, res_gamma=0.5
TAU       = 0.7
RES_GAMMA = 0.5
...
class ParallelRunner:
    ...
    def _launch(self, run, idx, total):
        ...
        # (used by S2-mnist-CNN.py at module top) resolves.
...
def print_summary(all_runs):
    print("\n" + "=" * 92)
    print("SUMMARY  [model=CNN]  last / max test accuracy per run")
    print("=" * 92)
    by_axis = {}
    for r in all_runs:
...
def main():
    ap = argparse.ArgumentParser(description="CNN @ MNIST sweep (per-task logs, in-GPU parallelism)")
    ...
    print(f"[INFO] model=CNN gpu={args.gpu} parallel={args.parallel} "
